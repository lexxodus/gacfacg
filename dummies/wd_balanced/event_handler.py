...
def get_recent_event_skill(pid, eid, interval=None):
    url = ROOT_URL + "event_skill/"
    params = {"pid": pid, "eid": eid, "interval": interval}
    response = requests.get(url, params=params).json()
    if response:
        skill_1 = response[0]["skill_points"]
        skill_2 = response[-1]["skill_points"]
        if skill_2 >= skill_1:
...
def get_recent_task_skill(pid, tid, interval=None):
    url = ROOT_URL + "task_skill/"
    params = {"pid": pid, "tid": tid, "interval": interval}
    response = requests.get(url, params=params).json()
    if response:
        skill_1 = response[0]["skill_points"]
        skill_2 = response[-1]["skill_points"]
